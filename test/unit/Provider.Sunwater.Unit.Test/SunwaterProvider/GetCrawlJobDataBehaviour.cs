...
{
    public class GetCrawlJobDataBehaviour : SunwaterProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(SunwaterConstants.KeyName.ApiKey, nameof(SunwaterCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task SunwaterCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Sunwater.Core.SunwaterCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
